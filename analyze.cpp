    ...
    case NodeKind::ExpK:
        ...
        case ExpKind::AssignK:
        {
            ...
            if (t->attr.op == '=')
            {
                ...
                // if (t->child[0] != NULL && t->child[0]->nodeKind == NodeKind::ExpK && t->child[0]->subkind.exp == ExpKind::OpK && t->attr.op == '[')
                // {
                // tmp = (TreeNode *)st->lookup(t->child[0]->child[0]->attr.string);
                // if (tmp != NULL)
                // tmp->isInit = true;
                // }
                // t->isInit = true;
